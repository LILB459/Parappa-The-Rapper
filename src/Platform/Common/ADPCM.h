		...
		namespace SPU
		{
			...
			*/
			...
			class Channel : public Audio::SoundSource
			{
				...
				public:
					// Channel interface
					Channel(Block *memory_p, size_t memory_length, size_t p, size_t loop)
					{
						// Set decode memory
						decode.SetMemory(memory_p, memory_length);
					...
					void Decode(unsigned long out_sample_rate, int16_t *out, size_t frames) override
					{
						...
						for (; on && i < frames; i++)
						{
							...
							// Perform resample
							size_t ri = ((subposition >> 32) & 0xFF0) >> 4;
